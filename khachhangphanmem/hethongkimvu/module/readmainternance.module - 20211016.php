...
if($sql != null && mysqli_num_rows($sql)>0)
{
	...
	$output .='
        ...
        if ($total_rowspan_product_old == 0) $output .='<td rowspan="'.($total_rowspan_product==0?1:$total_rowspan_product).'">'.$rows['dvtProduct'].'</td>';
        if ($total_rowspan_product_old == 0) $output .='<td rowspan="'.($total_rowspan_product==0?1:$total_rowspan_product).'">'.$rows['quantityPicture'].'</td>';


        $output .='<td>'.$rows['typePictureFree'].'</td>';
        ...
        $output .='<td>'.$rows['verPictureFree'].'</td>';
        $output .='<td>'.$rows['msPicrureFree'].'</td>';
        if ($total_rowspan_product_old == 0) $output .='<td rowspan="'.($total_rowspan_product==0?1:$total_rowspan_product).'">'.$rows['pricePicture'].'</td>';
